...
bool CanWriteTo(const NDArray &out_arr,
  ...
  bool add_same = in_mem->get_data_handle() == static_cast<const mkldnn::memory*>(
                                                    out_arr.GetMKLDNNData())->get_data_handle();
  bool pdesc_same = static_cast<const mkldnn::memory*>(out_arr.GetMKLDNNData())->get_desc() == desc
...
mkldnn_output_t CreateMKLDNNMem(const NDArray &out_arr,
  ...
  if (kAddTo == req) {
    ...
    mkldnn::memory *mem = static_cast<mkldnn::memory*>(
      const_cast<NDArray &>(out_arr).CreateMKLDNNData(&desc));
