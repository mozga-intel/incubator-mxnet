...
struct Is {
  const T d_;
  bool EQUAL(const std::initializer_list<T> &values) {
    return std::find(values.cbegin(), values.cend(), d_) != values.cend();
  }
  bool NOT_EQUAL(const std::initializer_list<T> &values) {
    return std::find(values.cbegin(), values.cend(), d_) != values.cend();
  }
};
...
inline bool check_support_mkldnn_pooling(const mkldnn::algorithm &alg_kind) {
  return alg_kind != mkldnn::algorithm::pooling_max &&
...
static inline mkldnn::memory::data_type get_data_type(
...
void MKLDNNPoolingFwd::Init(
  ...
  if (check_support_mkldnn_pooling(alg_kind)) {
...
void MKLDNNPoolingFwd::Execute(const NDArray &in_data, const OpReqType req,
  ...
  auto output_mem_t_ =
      CreateMKLDNNMem(out_data, this->fwd_pd_->dst_desc(), req);
...
void InitPoolingPrimitiveParams(const PoolingParam &param,
  ...
  if (kernel_ndims == 1) {
    ...
    if (param.pooling_convention == pool_enum::kFull) {
      pad_r[0] = GetPaddingSizeFull(data_md.data.dims[2], pad_l[0], pad_r[0],
                                    kernel[0], strides[0]);
    }
    ...
    if (param.pooling_convention == pool_enum::kFull) {
      pad_r[0] = GetPaddingSizeFull(data_md.data.dims[2], pad_l[0], pad_r[0],
                                    kernel[0], strides[0]);
      pad_r[1] = GetPaddingSizeFull(data_md.data.dims[3], pad_l[1], pad_r[1],
                                    kernel[1], strides[1]);
    }
    ...
    if (param.pooling_convention == pool_enum::kFull) {
      ...
      pad_r[1] = GetPaddingSizeFull(data_md.data.dims[3], pad_l[1], pad_r[1],
                                    kernel[1], strides[1]);
      pad_r[2] = GetPaddingSizeFull(data_md.data.dims[4], pad_l[2], pad_r[2],
                                    kernel[2], strides[2]);
    }
...
mkldnn::pooling_forward::primitive_desc GetPoolingFwdPdesc(
    const PoolingParam &param, const bool is_train,
    const mkldnn::memory::desc &data_md, const mkldnn::memory::desc &out_md) {
...
MKLDNNPoolingFwd &GetPoolingFwd(const PoolingParam &param, const bool is_train,
                                const NDArray &data, const NDArray &output) {
  ...
  static MX_THREAD_LOCAL
      std::unordered_map<MKLDNNPoolingSignature, MKLDNNPoolingFwd, OpHash>
          pooling_fwds;
#endif
  ...
  if (it == pooling_fwds.end()) {
    ...
    const mkldnn::algorithm alg = GetMKLDNNPoolAlgo(param);
    MKLDNNPoolingFwd fwd(data, output, kernel, strides, pad_l, pad_r, alg,
                         with_workspace, is_train);
...
#if DMLC_CXX11_THREAD_LOCAL
  static thread_local std::unordered_map<MKLDNNPoolingSignature,
                                         MKLDNNPoolingBwd, OpHash>
      pooling_bwds;
#else
  static MX_THREAD_LOCAL
      std::unordered_map<MKLDNNPoolingSignature, MKLDNNPoolingBwd, OpHash>
          pooling_bwds;
#endif
