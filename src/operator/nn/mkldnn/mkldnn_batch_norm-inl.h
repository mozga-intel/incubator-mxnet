...
void MKLDNNBatchNormForward(const nnvm::NodeAttrs &attrs, const OpContext &ctx,
  ...
  auto out_mem = static_cast<mkldnn::memory*>(
    const_cast<NDArray &>(out).CreateMKLDNNData(&fwd_dst_desc));
  ...
  if (static_cast<int>(flags) & static_cast<int>(mkldnn::normalization_flags::use_scale_shift)) {
    ...
    if (!ctx.is_train || param.use_global_stats) {
      ...
      net_args[MKLDNN_ARG_MEAN] = *(static_cast<const mkldnn::memory*>(
                            aux_states[batchnorm::kMovingMean].GetMKLDNNData()));
      net_args[MKLDNN_ARG_VARIANCE] = *(static_cast<const mkldnn::memory*>(
                            aux_states[batchnorm::kMovingVar].GetMKLDNNData()));
...
void MKLDNNBatchNormBackward(const nnvm::NodeAttrs &attrs, const OpContext &ctx,
  ...
  if (static_cast<int>(flags) & static_cast<int>(mkldnn::normalization_flags::use_scale_shift)) {
    ...
    if (fuse_relu) {
      ...
      workspace = &inputs[8];
      if (workspace != nullptr) {
        net_args[MKLDNN_ARG_WORKSPACE] = *(static_cast<const mkldnn::memory*>(
                                                                      workspace->GetMKLDNNData()));
      }
    }
