...
void AdaptiveAvgPoolComputeExCPU(const nnvm::NodeAttrs& attrs,
  ...
  /*
  DNNL doesn't support adaptive pooling. Fallback is needed when padding is not equal 0;
  */
  if(SupportMKLDNN(inputs[0]) && CanMKLDNNSupportAveragePooling(inputs[0], outputs[0])) {
