...
void SgMKLDNNConvOperator::Forward(const OpContext &ctx,
  ...
  if (!initialized_) {
    ...
    if (has_bias) args_[MKLDNN_ARG_BIAS] = *static_cast<const mkldnn::memory*>(
      cached_bias_.GetMKLDNNData());
  ...
  if (mkldnn_param.with_sum) {
    ...
    if (out_mem_desc != dst_mem_desc) {
      ...
      auto tmp_out_mem = static_cast<const mkldnn::memory*>(
        output.GetMKLDNNDataReorder(&fwd_dst_desc));
  ...
  if (mkldnn_param.quantized) {
    ...
    mkldnn::memory *mem = static_cast<mkldnn::memory*>(
      output.CreateMKLDNNData(&fwd_pd_dst_desc));
